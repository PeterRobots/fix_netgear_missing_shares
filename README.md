...
What you need: 
- The name of the data volume containing all the shares (check the webgui and shares, it's on the left in big text)![[netgear_share.png.png]]
...
4) List the contents of the share configuration folder for your missing share
	...
	- If the `.conf` files are mostly size 0, then they are corrupt and likely can be fixed with this method
	 ![[netgear_broken_share_configs.png]]
...
```bash
cd DATAVOLUMEPATH
wget http://www.github.com/PeterRobots/fix_netgear_nas_shares/main/fix_nas_shares.sh
